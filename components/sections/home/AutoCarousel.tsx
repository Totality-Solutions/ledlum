...
export default function CombinedLightingPartners({ posts }: { posts: any[] }) {
  const router = useRouter();
  const [currentIndex, setCurrentIndex] = useState(0);
  ...
  // Sync logic: Track which slide is active based on the interval
  useEffect(() => {
    if (LIGHTING_IMAGES.length === 0) return;
  ...
  const handleSliderClick = () => {
    if (BLOG_DATA[currentIndex]) {
      router.push(BLOG_DATA[currentIndex].link);
    }
  };
