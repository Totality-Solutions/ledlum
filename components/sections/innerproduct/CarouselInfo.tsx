...
export default function ProductInfoSection({ config, activeId, onModelChange, allModelIds, permutations = [] }: ProductInfoProps) {
  ...
  // Desktop carousel — only kicks in when allModelIds.length > 4
  const VISIBLE_COUNT = 4;
  const [carouselOffset, setCarouselOffset] = useState(0);
  const isCarousel = allModelIds.length > VISIBLE_COUNT;
  const canCarouselPrev = carouselOffset > 0;
  const canCarouselNext = carouselOffset + VISIBLE_COUNT < allModelIds.length;

  const handleCarouselPrev = () => {
    if (canCarouselPrev) setCarouselOffset((prev) => prev - 1);
  };
  const handleCarouselNext = () => {
    if (canCarouselNext) setCarouselOffset((prev) => prev + 1);
  };

  // Keep carousel window in sync when activeId changes externally
  useEffect(() => {
    if (!isCarousel) return;
    const idx = allModelIds.findIndex(id => id.toLowerCase() === activeId.toLowerCase());
    if (idx < carouselOffset) setCarouselOffset(idx);
    else if (idx >= carouselOffset + VISIBLE_COUNT) setCarouselOffset(idx - VISIBLE_COUNT + 1);
  ...
  // Original ModelOption card (unchanged from original)
    ...
    return (
      <div
        ...
        className={`h-[60px] md:h-[90px] flex items-center px-6 rounded-[12px] cursor-pointer transition-all border shrink-0 ${
          isActive ? "bg-white border-white" : "bg-[#0A0A0A] border-white/10 hover:border-white/30"
        }`}
      >
        <div className={`w-4 h-4 md:w-5 md:h-5 rounded-full border-2 flex items-center justify-center mr-4 ${isActive ? "border-[#4A61AD]" : "border-white/20"}`}>
          {isActive && <div className="w-2 md:w-2.5 h-2 md:h-2.5 rounded-full bg-[#4A61AD]" />}
        </div>
        <span className={`text-sm md:text-lg font-normal uppercase flex-1 ${isActive ? "text-black" : "text-[#888888]"}`}>{id}</span>
        <div className={`h-8 md:h-10 w-[1px] mx-4 ${isActive ? "bg-black/10" : "bg-white/10"}`} />
  ...
  // Desktop-only carousel arrow (only shown when allModelIds.length > 4)
  const DesktopCarouselArrow = ({ direction, onClick, disabled }: { direction: "left" | "right"; onClick: () => void; disabled: boolean }) => (
    <button
      onClick={onClick}
      disabled={disabled}
      ...
      <div className="">
        ...
        <div className="sticky top-0 z-[100] bg-black pt-6 pb-6 mb-10 -mx-4 px-4 md:-mx-[50px] md:px-[50px] border-b border-white/10 shadow-2xl">
          <div className="flex flex-col gap-8">
            ...
            {/* MOBILE & TABLET VIEW: original dropdown (unchanged) */}
            <div className="lg:hidden relative">
              ...
              >
                ...
                <motion.div animate={{ rotate: isMenuOpen ? 180 : 0 }} className="text-white/40">
                      ...
                      <div className="flex flex-col max-h-[400px] overflow-y-auto p-2 gap-1 custom-scrollbar">
                        {allModelIds.map((id) => (
                          <ModelOption
                            ...
                            id={id}
                            activeId={activeId}
                            onClick={(newId) => { onModelChange(newId); setIsMenuOpen(false); }}
                          />
                        ))}
                      </div>
                    </motion.div>
            ...
            {/* DESKTOP VIEW: grid (≤4 models) or carousel (>4 models) */}
            <div className="hidden lg:block">
              {!isCarousel ? (
                ...
                /* Carousel: show 4 at a time, slide by 1 */
                <div className="flex items-center gap-4">
                  <DesktopCarouselArrow direction="left" onClick={handleCarouselPrev} disabled={!canCarouselPrev} />

                  <div className="flex-1 grid grid-cols-4 gap-5 overflow-hidden">
                    {allModelIds.slice(carouselOffset, carouselOffset + VISIBLE_COUNT).map((id) => (
                      <ModelOption key={id} id={id} activeId={activeId} onClick={onModelChange} />
                    ))}
                  </div>

                  <DesktopCarouselArrow direction="right" onClick={handleCarouselNext} disabled={!canCarouselNext} />
                </div>
              )}
            </div>
          </div>
