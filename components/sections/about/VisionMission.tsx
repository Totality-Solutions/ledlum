...
export default function VisionMission() {
  ...
  return (
    <Section className="relative bg-black text-white py-24 md:py-32 lg:py-52 overflow-hidden font-bai">
      ...
      <div
        className="absolute inset-0 z-0 opacity-25 bg-[url('/images/about/ledlumline.png')] bg-cover bg-center pointer-events-none will-change-transform"
        style={{
      ...
      {/* MISSION CIRCULAR IMAGE */}
      <div className="absolute top-[20%] left-[-10%] md:left-[5%] 
                      w-[400px] h-[400px] md:w-[700px] md:h-[700px] 
                      z-0 pointer-events-none">

        <div
          className="w-full h-full rounded-full opacity-20 md:opacity-90"
          style={{
            ...
            transform: 'scale(1.5)',   // reduced from 1.7
...
function ValueItem({ label, className, lineHeight, orientation }: ValueItemProps) {
  return (
    <div className={`flex flex-col items-center ${className}`}>
      {orientation === 'top' && (
        <>
          <span className="body-sm !text-[10px] md:!text-[11px] tracking-[0.2em] !text-zinc-500 mb-4 text-center font-light max-w-[120px]">
            {label}
          </span>
          <div className="w-1.5 h-1.5 rounded-full bg-white shadow-[0_0_6px_white]" />
        </>
      )}
      <div className={`w-[1px] ${lineHeight} bg-gradient-to-b from-white/40 via-white/10 to-transparent`} />
      {orientation === 'bottom' && (
        <>
          <div className="w-1.5 h-1.5 rounded-full bg-white shadow-[0_0_6px_white]" />
          <span className="body-sm !text-[10px] md:!text-[11px] tracking-[0.2em] !text-zinc-500 mt-4 text-center font-light max-w-[120px]">
            {label}
          </span>
        </>
      )}
