...
export default function RequestQuotePopup({ product, onClose }: RequestQuotePopupProps) {
      ...
      >
        ...
        {submitted ? (
          <div className="py-6 text-center">
            <h3 className="text-xl font-pop font-semibold text-white mb-2">Request Sent</h3>
            <p className="text-sm font-pop font-regular text-white/50 mb-6">
              Thanks {name.split(" ")[0]}, our team will get back to you shortly with a quote.
            </p>
            <button
              onClick={onClose}
              className="w-full py-3 rounded-full bg-[#8D794E] hover:bg-[#9a8c66] text-white text-sm font-pop font-medium transition-colors"
            >
              Close
            </button>
          </div>
          ...
          <>
            <h3 className="text-xl font-pop font-semibold text-white mb-1">Request Quote</h3>
            <p className="text-sm font-pop font-regular text-white/50 mb-6">
              {product ? (
                <>Share your details and we&apos;ll send you a quote for <span className="text-white/80">{product}</span>.</>
              ) : (
                <>Share your details and our team will send you a custom quote.</>
              )}
            </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <textarea
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-3 text-sm text-white placeholder-white/30 focus:outline-none focus:border-[#8D794E]/50 transition-all resize-none"
                />
              </div>
