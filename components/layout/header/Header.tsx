  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          <nav className="hidden lg:flex flex-1 justify-center items-center gap-8 xl:gap-14 mx-8">
            {primaryNavigation.map((link) => {
              // Exact match for homepage, or partial match for sub-routes
              ...
              return (
                <Link
                  ...
                  href={link.href}
                  onClick={closeAll}
                  className="relative group transition-all duration-300 py-2"
                >
                  <span 
                    className={`body-sm font-medium transition-colors duration-300 ${
                      isActive ? "text-[#AD9463]" : "text-white/80 group-hover:text-[#AD9463]"
                    }`}
                  >
                    {link.title}
                  </span>
                  ...
                  <div 
                    className={`absolute bottom-0 left-0 h-0.5 bg-gradient-to-r from-[#AD9463] to-transparent transition-all duration-300 ${
        ...
        >
          <div className="bg-[#111111] w-full shadow-2xl border border-white/5 overflow-hidden rounded-2xl">
            <div className="p-7 flex flex-wrap justify-center content-start gap-4 ">
              {sideNavigation.map((item, index) => {
                const isSideActive = pathname === item.href || (item.href !== "/" && pathname.startsWith(item.href));
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    onClick={closeAll}
                    className={`h-auto w-[150px] p-4 flex-shrink-0 border flex items-center justify-center px-6 transition-all cursor-pointer group ${
                      isSideActive 
                        ? "bg-[#AD9463]/20 border-[#AD9463] rounded-lg" 
                        : "bg-[#222222] border-white/5 hover:bg-[#AD9463]/20 hover:border-[#AD9463]/40 rounded-lg"
                    }`}
                  >
                    <span 
                      className={`text-content body-sm text-center whitespace-nowrap transition-colors ${
                        isSideActive ? "text-white font-medium" : "group-hover:text-white"
                      }`}
                    >
                      {item.title}
                    </span>
