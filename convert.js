...
// 1. PASTE YOUR ENTIRE ARRAY HERE
const blogPosts = [ 
    {
...
const ndjson = blogPosts.map(post => ({
  _type: 'post',
  title: post.title,
  ...
  image: post.image,
  midSectionTitle: post.midSectionTitle,
  paragraph: post.paragraph,
  midSectionImage: post.midSectionImage,
  outcomeSections: post.outcomeSections,
  date: post.date,
  isFeatured: post.isFeatured || false
})).map(JSON.stringify).join('\n');

fs.writeFileSync('data.ndjson', ndjson);
console.log('Success! data.ndjson has been created in your root folder.');