...
export default async function BlogPost({ params }: PageProps) {
  ...
  const heroImage = post.image || "/images/placeholder.jpg";
  const midSectionImage = post.midSectionImage || "/images/placeholder.jpg";
            ...
            <section className="">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-16 md:gap-y-20">
                {latestInsights.map((insight: any) => (
                  <a key={insight.slug.current} href={`/blog/${insight.slug.current}`}>
                    <BlogCard
                      title={insight.title}
                      category={insight.category}
                      description={insight.description}
                      image={insight.image || "/images/placeholder.jpg"}
                      date={insight.date}
                    />
                  </a>
                ))}
